...
static int initialize(void)
{
	...
	{
		printf("ERROR: clGetPlatformIDs(0,0,*) failed\n");
		return -1;
	}
	cl_platform_id all_platform_id[num_platforms];
	if (clGetPlatformIDs(num_platforms, all_platform_id, NULL) != CL_SUCCESS)
	{
		printf("ERROR: clGetPlatformIDs(*,*,0) failed\n");
		return -1;
	}
	...
	if (clGetDeviceIDs(platform_id, CL_DEVICE_TYPE_ALL, num_devices, device_list, NULL) != CL_SUCCESS)
	{
		printf("ERROR: clGetDeviceIDs failed\n");
		return -1;
	};
	...
	// get device type
	if (clGetDeviceInfo(device_list[device_id_inuse], CL_DEVICE_TYPE, sizeof(device_type), (void *)&device_type, NULL) != CL_SUCCESS)
	{
		printf("ERROR: clGetDeviceIDs failed\n");
		return -1;
	};
...
int bpnn_train_kernel(BPNN *net, float *eo, float *eh)
{
	...
	if (err != CL_SUCCESS)
	{
		printf("ERROR: clEnqueueWriteBuffer input_ocl\n");
		return -1;
	}
	...
	if (err != CL_SUCCESS)
	{
		printf("ERROR: clEnqueueWriteBuffer input_hidden_ocl\n");
		return -1;
	}
#ifdef TIMING
	...
	if (err != CL_SUCCESS)
	{
		printf("ERROR: 1  clEnqueueReadBuffer: partial sum\n");
		return -1;
	}
#ifdef TIMING
	...
	if (err != CL_SUCCESS)
	{
		printf("ERROR: clEnqueueWriteBuffer hidden_delta_ocl\n");
		return -1;
	}
	...
	if (err != CL_SUCCESS)
	{
		printf("ERROR: clEnqueueWriteBuffer input_prev_weights_ocl\n");
		return -1;
	}
	...
	if (err != CL_SUCCESS)
	{
		printf("ERROR: 1  clEnqueueReadBuffer: input_ocl\n");
		return -1;
	}
#ifdef TIMING
	d2h_time += probe_event_time(event, cmd_queue);
	...
	if (err != CL_SUCCESS)
	{
		printf("ERROR: 1  clEnqueueReadBuffer: input_hidden_ocl\n");
		return -1;
	}
#ifdef TIMING
	d2h_time += probe_event_time(event, cmd_queue);
