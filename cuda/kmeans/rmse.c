...
float rms_err(float **feature,                                     /* [npoints][nfeatures] */
              int nfeatures, int npoints, float **cluster_centres, /* [nclusters][nfeatures] */
              int nclusters) {
  int i;
...
#pragma omp parallel for shared(feature, cluster_centres)                                                              \
    firstprivate(npoints, nfeatures, nclusters) private(i, nearest_cluster_index) schedule(static)
  for (i = 0; i < npoints; i++) {
