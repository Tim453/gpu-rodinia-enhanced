...
void initHistogram1024(void) { checkCudaErrors(cudaMalloc((void **)&d_Result1024, HISTOGRAM_SIZE)); }

// Internal memory deallocation
void closeHistogram1024(void) { checkCudaErrors(cudaFree(d_Result1024)); }

// histogram1024 CPU front-end
void histogram1024GPU(unsigned int *h_Result, float *d_Data, float minimum, float maximum, int dataN) {
  checkCudaErrors(cudaMemset(d_Result1024, 0, HISTOGRAM_SIZE));
  histogram1024Kernel<<<BLOCK_N, THREAD_N>>>(d_Result1024, d_Data, minimum, maximum, dataN);
