...
#define RENEW(var, num, type)                                                                                          \
  ((var) = (type *)((var) ? realloc((char *)(var), (size_t)(num) * sizeof(type)) : calloc((size_t)(num), sizeof(type))))

#define MEMCOPY(from, to, n_items, type) MEM_COPY((char *)(from), (char *)(to), (size_t)(n_items) * sizeof(type))
...
/* re-allocate arry to have num objects of the given type */
#define RENEW(var, num, type)                                                                                          \
  ((var) = (type *)((var) ? realloc((char *)(var), (size_t)((num) * sizeof(type)))                                     \
...
#define bd_chk_idx(A, i, j)                                                                                            \
  ((i) >= max(0, (j) - (A)->ub) && (j) >= max(0, (i) - (A)->lb) && (i) < (A)->mat->n && (j) < (A)->mat->n)

#define m_entry(A, i, j) m_get_val(A, i, j)
#define v_entry(x, i) v_get_val(x, i)
#define bd_entry(A, i, j) bd_get_val(A, i, j)
#ifdef DEBUG
#define m_set_val(A, i, j, val) (m_chk_idx(A, i, j) ? (A)->me[(i)][(j)] = (val) : (error(E_BOUNDS, "m_set_val"), 0.0))
#define m_add_val(A, i, j, val) (m_chk_idx(A, i, j) ? (A)->me[(i)][(j)] += (val) : (error(E_BOUNDS, "m_add_val"), 0.0))
#define m_sub_val(A, i, j, val) (m_chk_idx(A, i, j) ? (A)->me[(i)][(j)] -= (val) : (error(E_BOUNDS, "m_sub_val"), 0.0))
#define m_get_val(A, i, j) (m_chk_idx(A, i, j) ? (A)->me[(i)][(j)] : (error(E_BOUNDS, "m_get_val"), 0.0))
#define v_set_val(x, i, val) (v_chk_idx(x, i) ? (x)->ve[(i)] = (val) : (error(E_BOUNDS, "v_set_val"), 0.0))
#define v_add_val(x, i, val) (v_chk_idx(x, i) ? (x)->ve[(i)] += (val) : (error(E_BOUNDS, "v_set_val"), 0.0))
#define v_sub_val(x, i, val) (v_chk_idx(x, i) ? (x)->ve[(i)] -= (val) : (error(E_BOUNDS, "v_set_val"), 0.0))
#define v_get_val(x, i) (v_chk_idx(x, i) ? (x)->ve[(i)] : (error(E_BOUNDS, "v_get_val"), 0.0))
#define bd_set_val(A, i, j, val)                                                                                       \
  (bd_chk_idx(A, i, j) ? (A)->mat->me[(A)->lb + (j) - (i)][(j)] = (val) : (error(E_BOUNDS, "bd_set_val"), 0.0))
#define bd_add_val(A, i, j, val)                                                                                       \
  (bd_chk_idx(A, i, j) ? (A)->mat->me[(A)->lb + (j) - (i)][(j)] += (val) : (error(E_BOUNDS, "bd_set_val"), 0.0))
#define bd_get_val(A, i, j)                                                                                            \
  (bd_chk_idx(A, i, j) ? (A)->mat->me[(A)->lb + (j) - (i)][(j)] : (error(E_BOUNDS, "bd_get_val"), 0.0))
#else /* no DEBUG */
...
#define finput(fp, prompt, fmt, var)                                                                                   \
  ((isatty(fileno(fp)) ? fprintf(stderr, prompt) : skipjunk(fp)), fscanf(fp, fmt, var))
...
extern VEC *_v_copy(const VEC *in, VEC *out, unsigned int i0), *v_move(const VEC *in, int, int, VEC *out, int),
    *mv_move(const MAT *in, int, int, int, int, VEC *out, int);
extern PERM *px_copy(const PERM *in, PERM *out);
extern IVEC *iv_copy(const IVEC *in, IVEC *out), *iv_move(const IVEC *in, int, int, IVEC *out, int);
...
#ifndef ANSI_C
extern VEC *sv_mlt(), *mv_mlt(), *vm_mlt(), *v_add(), *v_sub(), *px_vec(), *pxinv_vec(), *v_mltadd(), *v_map(),
    *_v_map(), *v_lincomb(), *v_linlist();
extern double v_min(), v_max(), v_sum();
extern VEC *v_star(), *v_slash(), *v_sort();
extern double _in_prod(), __ip__();
extern void __mltadd__(), __add__(), __sub__(), __smlt__(), __zero__();
#else
...
extern void __mltadd__(Real *, const Real *, double, int), __add__(const Real *, const Real *, Real *, int),
    __sub__(const Real *, const Real *, Real *, int), __smlt__(const Real *, double, Real *, int),
    __zero__(Real *, int);
...
extern double _v_norm1(), _v_norm2(), _v_norm_inf(), m_norm1(), m_norm_inf(), m_norm_frob();
...
extern double m_norm1(const MAT *A), m_norm_inf(const MAT *A), m_norm_frob(const MAT *A);
...
extern MAT *sm_mlt(), *m_mlt(), *mmtr_mlt(), *mtrm_mlt(), *m_add(), *m_sub(), *sub_mat(), *m_transp(), *ms_mltadd();

extern BAND *bd_transp(), *sbd_mlt(), *bds_mltadd(), *bd_zero();
extern MAT *px_rows(), *px_cols(), *swap_rows(), *swap_cols(), *_set_row(), *_set_col();
extern VEC *get_row(), *get_col(), *sub_vec(), *mv_mltadd(), *vm_mltadd(), *bdv_mltadd();
...
extern MAT *sm_mlt(double s, const MAT *A, MAT *out), /* out <- s.A */
    ...
    *sub_mat(const MAT *A, unsigned int, unsigned int, unsigned int, unsigned int, MAT *out),
...
extern MAT *px_rows(const PERM *px, const MAT *A, MAT *out), /* out <- P.A */
    ...
    *swap_rows(MAT *, int, int, int, int), *swap_cols(MAT *, int, int, int, int),
    /* A[i][j] <- out[j], j >= j0 */
    *_set_col(MAT *A, unsigned int i, const VEC *col, unsigned int j0),
    /* A[i][j] <- out[i], i >= i0 */
    *_set_row(MAT *A, unsigned int j, const VEC *row, unsigned int i0);

extern VEC *get_row(const MAT *, unsigned int, VEC *), *get_col(const MAT *, unsigned int, VEC *),
    ...
    *bdv_mltadd(const VEC *x, const VEC *y, const BAND *A, double s, VEC *out);
#endif /* ANSI_C */
...
void m_dump(FILE *fp, const MAT *a), px_dump(FILE *fp, const PERM *px), v_dump(FILE *fp, const VEC *x),
    iv_dump(FILE *fp, const IVEC *ix);
