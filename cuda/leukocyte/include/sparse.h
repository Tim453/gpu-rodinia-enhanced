...
#define sprow_idx2(r, c, hint)                                                                                         \
  (((hint) >= 0 && (hint) < (r)->len && (r)->elt[hint].col == (c)) ? (hint) : sprow_idx((r), (c)))
...
#ifndef ANSI_C
extern SPMAT *sp_get(), *sp_copy(), *sp_copy2(), *sp_zero(), *sp_resize(), *sp_compact();
...
extern SPROW *sprow_get(), *sprow_xpd(), *sprow_merge(), *sprow_mltadd(), *sprow_resize(), *sprow_copy();
...
#else
SPMAT *sp_get(int, int, int), *sp_copy(const SPMAT *), *sp_copy2(const SPMAT *, SPMAT *), *sp_zero(SPMAT *),
    *sp_resize(SPMAT *, int, int), *sp_compact(SPMAT *, double);
double sp_get_val(const SPMAT *, int, int), sp_set_val(SPMAT *, int, int, double);
VEC *sp_mv_mlt(const SPMAT *, const VEC *, VEC *), *sp_vm_mlt(const SPMAT *, const VEC *, VEC *);
int sp_free(SPMAT *);
...
/* algebraic operations */
SPMAT *sp_smlt(const SPMAT *A, double alpha, SPMAT *B), *sp_add(const SPMAT *A, const SPMAT *B, SPMAT *C),
    *sp_sub(const SPMAT *A, const SPMAT *B, SPMAT *C),
    *sp_mltadd(const SPMAT *A, const SPMAT *B, double alpha, SPMAT *C);

/* sparse row operations */
SPROW *sprow_get(int), *sprow_xpd(SPROW *r, int n, int type), *sprow_resize(SPROW *r, int n, int type),
    *sprow_merge(const SPROW *, const SPROW *, SPROW *, int type),
    *sprow_copy(const SPROW *, const SPROW *, SPROW *, int type),
    *sprow_mltadd(const SPROW *r1, const SPROW *r2, double alpha, int j0, SPROW *r_out, int type);
...
#define fixindex(idx) ((idx) == -1 ? (error(E_BOUNDS, "fixindex"), 0) : (idx) < 0 ? -((idx) + 2) : (idx))
