...
__global__ void IMGVF_kernel(float **IMGVF_array, float **I_array, int *m_array, int *n_array, float vx, float vy,
                             float e, int max_iterations, float cutoff) {
  ...
  while ((!cell_converged) && (iterations < max_iterations)) {
    ...
    for (thread_block = 0; thread_block < max; thread_block++) {
      ...
      if (i < m) {
        ...
        new_val = old_val + (MU / LAMBDA) *
                                (UHe * U + DHe * D + LHe * L + RHe * R + URHe * UR + DRHe * DR + ULHe * UL + DLHe * DL);
...
void IMGVF_cuda(MAT **I, MAT **IMGVF, double vx, double vy, double e, int max_iterations, double cutoff,
  ...
  IMGVF_kernel<<<num_cells, threads_per_block>>>(device_IMGVF_array, device_I_array, device_m_array, device_n_array,
