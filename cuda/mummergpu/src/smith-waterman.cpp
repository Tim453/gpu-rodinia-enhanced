...
void align(const string &S, const string &T, int match_score, int mismatch_score, int gap_open, int gap_extension) {
  int m = S.length() - 1;
  int n = T.length() - 1;
  ...
  for (int i = 1; i <= m; i++) {
    for (int j = 1; j <= n; j++) {
      E[i][j] = max2(E[i - 1][j] - gap_extension, V[i - 1][j] - gap_open - gap_extension);

      F[i][j] = max2(F[i - 1][j] - gap_extension, V[i - 1][j] - gap_open - gap_extension);
