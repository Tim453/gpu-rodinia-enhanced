...
// Host-side handles for the texture objects pushed into the device symbols of
// the same name in mummergpu_kernel.cu, kept so they can be destroyed again.
static cudaTextureObject_t h_reftex = 0;
static cudaTextureObject_t h_nodetex = 0;
static cudaTextureObject_t h_childrentex = 0;
static cudaTextureObject_t h_qrytex = 0;

// All four textures used the same sampling settings under the old texture
// reference API: clamped addressing, point filtering, unnormalized coordinates.
static cudaTextureDesc mummerTextureDesc() {
...
static cudaTextureObject_t createLinearTexture(void *arr, cudaChannelFormatDesc desc, size_t len) {
  cudaResourceDesc resDesc;
  memset(&resDesc, 0, sizeof(resDesc));
  resDesc.resType = cudaResourceTypeLinear;
  resDesc.res.linear.devPtr = arr;
  resDesc.res.linear.desc = desc;
  resDesc.res.linear.sizeInBytes = len;

  cudaTextureDesc texDesc = mummerTextureDesc();
  cudaTextureObject_t tex = 0;
  CUDA_SAFE_CALL(cudaCreateTextureObject(&tex, &resDesc, &texDesc, NULL));
  return tex;
}

static cudaTextureObject_t createArrayTexture(cudaArray *arr) {
  cudaResourceDesc resDesc;
  memset(&resDesc, 0, sizeof(resDesc));
...
// `tex` names both the __device__ symbol in the kernel and the h_ handle here.
#define BIND_TEX(offset, tex, arr, desc, len)                                                                          \
  do {                                                                                                                 \
    h_##tex = createLinearTexture((void *)(arr), desc, len);                                                           \
    CUDA_SAFE_CALL(cudaMemcpyToSymbol(tex, &h_##tex, sizeof(h_##tex)));                                                \
    ++num_bind_tex_calls;                                                                                              \
  } while (0)

#define BIND_TEX_ARRAY(tex, arr, desc)                                                                                 \
  do {                                                                                                                 \
    h_##tex = createArrayTexture((cudaArray *)(arr));                                                                  \
    CUDA_SAFE_CALL(cudaMemcpyToSymbol(tex, &h_##tex, sizeof(h_##tex)));                                                \
    ++num_bind_tex_calls;                                                                                              \
  } while (0)

#define UNBIND_TEX(tex)                                                                                                \
  do {                                                                                                                 \
    if (h_##tex) {                                                                                                     \
      CUDA_SAFE_CALL(cudaDestroyTextureObject(h_##tex));                                                               \
      h_##tex = 0;                                                                                                     \
    }                                                                                                                  \
  } while (0)
...
extern "C" void computeGold(MatchResults *results, char *refstr, char *queries, int *queryAddrs, int *queryLengths,
                            PixelOfNode *nodeTexture, PixelOfChildren *childrenTexture, int numQueries,
                            int mismatch_length, int rc);

extern "C" void getReferenceString(const char *filename, char **refstr, size_t *reflen);

extern "C" void createTreeTexture(const char *filename, PixelOfNode **nodeTexture, PixelOfChildren **childrenTexture,
                                  unsigned int *width, unsigned int *node_height, unsigned int *children_height,
                                  AuxiliaryNodeData **aux_data, int *num_match_coords, int min_match_len,
                                  Statistics *statistics, const char *dotfilename, const char *texfilename);

extern "C" void getQueriesTexture(int qfile, char **queryTexture, size_t *queryLength, int **queryAddrs,
                                  char ***queryNames, int **queryLengths, unsigned int *numQueries,
                                  unsigned int *num_match_coords, unsigned int device_memory_avail,
                                  int min_match_length, bool rc);

extern "C" int lookupNumLeaves(ReferencePage *page, TextureAddress addr);

void printAlignments(ReferencePage *page, Alignment *alignments, char *query, int qrylen, TextureAddress nodeid,
                     int qrypos, int edge_depth, int min_match, bool rc, bool forwardcoordinates);

int countLeafNodes(int nodeid);

extern "C" void mapQueriesEndToEnd(MatchContext *ctx, ReferencePage *page, MatchInfo *h_matches,
                                   unsigned int numMatches, Alignment *h_alignments, unsigned int numAligments);
...
void startTimer(char *ptr) { gettimeofday(&(((struct Timer_t *)ptr)->start_m), NULL); }

void stopTimer(char *ptr) { gettimeofday(&(((struct Timer_t *)ptr)->end_m), NULL); }
...
extern "C" int createMatchContext(Reference *ref, QuerySet *queries, MatchResults *matches, bool on_cpu,
                                  int min_match_length, char *stats_file, bool reverse, bool forwardreverse,
                                  bool forwardcoordinates, bool showQueryLength, char *dotfilename, char *texfilename,
                                  MatchContext *ctx) {
...
void buildReferenceTexture(Reference *ref, char *full_ref, size_t begin, size_t end, int min_match_len,
                           char *dotfilename, char *texfilename, Statistics *statistics) {
  ...
  createTreeTexture(ref->str, &nodeTexture, &childrenTexture, &width, &node_height, &children_height, &aux_data,
                    &num_nodes, min_match_len, statistics, dotfilename, texfilename);
  ...
  ref->bytes_on_board =
      (width * node_height * sizeof(PixelOfNode)) + (width * children_height * sizeof(PixelOfChildren));
...
void loadReferenceTexture(MatchContext *ctx) {
  ...
  if (!ctx->on_cpu) {
    ...
    CUDA_MALLOC_ARRAY((cudaArray **)(&ref->d_ref_array), &refTextureDesc, ref->pitch, numrows);

    CUDA_SAFE_CALL(cudaMemcpy2DToArray((cudaArray *)(ref->d_ref_array), 0, 0, ref->h_ref_array, ref->pitch, ref->pitch,
    ...
    CUDA_SAFE_CALL(cudaMemcpy((void *)(ref->d_ref_array), ref->str, ref->len, cudaMemcpyHostToDevice));
    ...
    CUDA_MALLOC_PITCH((void **)(&ref->d_ref_array), &refpitch, ref->pitch * sizeof(char), numrows);
    CUDA_SAFE_CALL(cudaMemcpy2D((ref->d_ref_array), refpitch, ref->h_ref_array, ref->pitch, ref->pitch * sizeof(char),
                                numrows, cudaMemcpyHostToDevice));
...
void loadReference(MatchContext *ctx) {
  ...
  if (!ctx->on_cpu) {
    ...
    CUDA_MALLOC_ARRAY((cudaArray **)(&ref->d_node_tex_array), &nodeTextureDesc, ref->tex_width, ref->tex_node_height);

    CUDA_SAFE_CALL(cudaMemcpy2DToArray((cudaArray *)(ref->d_node_tex_array), 0, 0, ref->h_node_tex_array,
                                       ref->tex_width * sizeof(PixelOfNode), ref->tex_width * sizeof(PixelOfNode),
                                       ref->tex_node_height, cudaMemcpyHostToDevice));

    BIND_TEX_ARRAY(nodetex, (cudaArray *)ref->d_node_tex_array, nodeTextureDesc);
#else
    size_t nodepitch;

    CUDA_MALLOC_PITCH((void **)(&ref->d_node_tex_array), &nodepitch, ref->tex_width * sizeof(PixelOfNode),
                      ref->tex_node_height);
    CUDA_SAFE_CALL(cudaMemcpy2D((ref->d_node_tex_array), nodepitch, ref->h_node_tex_array, nodepitch,
                                ref->tex_width * sizeof(PixelOfNode), ref->tex_node_height, cudaMemcpyHostToDevice));
    ...
    if (ref->tex_children_height) {
      ...
      CUDA_MALLOC_ARRAY((cudaArray **)(&ref->d_children_tex_array), &childrenTextureDesc, ref->tex_width,
                        ref->tex_children_height);

      CUDA_SAFE_CALL(cudaMemcpy2DToArray((cudaArray *)(ref->d_children_tex_array), 0, 0, ref->h_children_tex_array,
                                         ref->tex_width * sizeof(PixelOfChildren),
                                         ref->tex_width * sizeof(PixelOfChildren), ref->tex_children_height,
                                         cudaMemcpyHostToDevice));

      BIND_TEX_ARRAY(childrentex, (cudaArray *)(ref->d_children_tex_array), childrenTextureDesc);
#else
      size_t childpitch;

      CUDA_MALLOC_PITCH((void **)(&ref->d_children_tex_array), &childpitch, ref->tex_width * sizeof(PixelOfChildren),
                        ref->tex_children_height);
      CUDA_SAFE_CALL(cudaMemcpy2D((ref->d_children_tex_array), childpitch, ref->h_children_tex_array, childpitch,
                                  ref->tex_width * sizeof(PixelOfNode), ref->tex_children_height,
                                  cudaMemcpyHostToDevice));
#endif
    }
    ...
    CUDA_MALLOC((void **)(&ref->d_node_hist), ref->tex_width * ref->tex_node_height * sizeof(int));
    CUDA_SAFE_CALL(cudaMemset((ref->d_node_hist), 0, ref->tex_width * ref->tex_node_height * sizeof(int)));

    if (ref->tex_children_height) {
      // children hist
      ref->bytes_on_board += ref->tex_width * ref->tex_children_height * sizeof(int);
      fprintf(stderr, "after child_hist  ref->bytes_on_board:%ld\n", ref->bytes_on_board);
      CUDA_MALLOC((void **)(&ref->d_child_hist), ref->tex_width * ref->tex_children_height * sizeof(int));
      CUDA_SAFE_CALL(cudaMemset((ref->d_child_hist), 0, ref->tex_width * ref->tex_children_height * sizeof(int)));
    }
#endif
    ...
    CUDA_MALLOC((void **)(&ref->d_node_tex_array), ref->tex_node_height * sizeof(PixelOfNode));

    CUDA_SAFE_CALL(cudaMemcpy((ref->d_node_tex_array), ref->h_node_tex_array,
                              ref->tex_node_height * sizeof(PixelOfNode), cudaMemcpyHostToDevice));
#if NODETEX
    cudaChannelFormatDesc nodeTextureDesc = cudaCreateChannelDesc(32, 32, 32, 32, cudaChannelFormatKindUnsigned);

    BIND_TEX(0, nodetex, (void *)(ref->d_node_tex_array), nodeTextureDesc, ref->tex_node_height * sizeof(PixelOfNode));
#endif
    if (ref->tex_children_height) {
      // Child tex, 1-dimensional
      CUDA_MALLOC((void **)(&ref->d_children_tex_array), ref->tex_children_height * sizeof(PixelOfChildren));

      CUDA_SAFE_CALL(cudaMemcpy((ref->d_children_tex_array), ref->h_children_tex_array,
                                ref->tex_children_height * sizeof(PixelOfChildren), cudaMemcpyHostToDevice));
#if CHILDTEX
      cudaChannelFormatDesc childTextureDesc = cudaCreateChannelDesc(32, 32, 32, 32, cudaChannelFormatKindUnsigned);

      BIND_TEX(0, childrentex, (void *)(ref->d_children_tex_array), childTextureDesc,
    ...
    CUDA_MALLOC((void **)(&ref->d_node_hist), ref->tex_node_height * sizeof(int));
    CUDA_SAFE_CALL(cudaMemset((ref->d_node_hist), 0, ref->tex_node_height * sizeof(int)));

    if (ref->tex_children_height) {
      ref->bytes_on_board += ref->tex_children_height * sizeof(int);
      CUDA_MALLOC((void **)(&ref->d_child_hist), ref->tex_children_height * sizeof(int));
      CUDA_SAFE_CALL(cudaMemset((ref->d_child_hist), 0, ref->tex_children_height * sizeof(int)));
    }
#endif
...
void loadQueries(MatchContext *ctx) {
  ...
  if (!ctx->on_cpu) {
    ...
    CUDA_SAFE_CALL(cudaMemcpy((void *)queries->d_tex_array, queries->h_tex_array + queries->h_addrs_tex_array[0],
    ...
    BIND_TEX(0, qrytex, (void *)(queries->d_tex_array), qryDesc, queries->texlen);
#endif
    CUDA_MALLOC((void **)&queries->d_addrs_tex_array, numQueries * sizeof(int));

    queries->bytes_on_board += numQueries * sizeof(int);

    CUDA_SAFE_CALL(cudaMemcpy((void *)queries->d_addrs_tex_array, queries->h_addrs_tex_array, numQueries * sizeof(int),
                              cudaMemcpyHostToDevice));

    CUDA_MALLOC((void **)&queries->d_lengths_array, numQueries * sizeof(int));

    queries->bytes_on_board += numQueries * sizeof(int);

    CUDA_SAFE_CALL(cudaMemcpy((void *)queries->d_lengths_array, queries->h_lengths_array, numQueries * sizeof(int),
                              cudaMemcpyHostToDevice));
    stopTimer(toboardtimer);
...
// kernel.
void buildCoordOffsetArray(MatchContext *ctx, int **h_coord_offset_array, unsigned int *num_coords) {
  int numCoords = 0;
...
void loadResultBuffer(MatchContext *ctx) {
  ...
  if (!ctx->on_cpu) {
    ...
    CUDA_MALLOC((void **)&ctx->results.d_match_coords, numCoords * sizeof(MatchCoord));
    ctx->results.bytes_on_board += numCoords * sizeof(MatchCoord);

    CUDA_SAFE_CALL(cudaMemset((void *)ctx->results.d_match_coords, 0, numCoords * sizeof(MatchCoord)));

#if COALESCED_QUERIES
    CUDA_MALLOC((void **)&ctx->results.d_coord_tex_array, numQueries * sizeof(int));

    ctx->results.bytes_on_board += numQueries * sizeof(int);

    CUDA_SAFE_CALL(cudaMemcpy((void *)ctx->results.d_coord_tex_array, ctx->results.h_coord_tex_array,
...
void transferResultsFromDevice(MatchContext *ctx) {
  if (!ctx->on_cpu) {
    ...
    CUDA_SAFE_CALL(cudaMemcpy(ctx->results.h_match_coords, ctx->results.d_match_coords,
                              ctx->results.numCoords * sizeof(MatchCoord), cudaMemcpyDeviceToHost));

#if TREE_ACCESS_HISTOGRAM
    CUDA_SAFE_CALL(cudaMemcpy(ctx->ref->h_node_hist, ctx->ref->d_node_hist,
                              ctx->ref->tex_node_height * ctx->ref->tex_width * sizeof(int), cudaMemcpyDeviceToHost));

    CUDA_SAFE_CALL(cudaMemcpy(ctx->ref->h_child_hist, ctx->ref->d_child_hist,
...
MatchCoord *coordForQueryChar(MatchContext *ctx, unsigned int qryid, unsigned int qrychar) {
...
void coordsToPrintBuffers(MatchContext *ctx, ReferencePage *page, MatchInfo **matches, Alignment **alignments,
                          unsigned int mem_avail, unsigned int *coord_idx, unsigned int *match_idx,
                          unsigned int *align_idx, unsigned int *nextqry, unsigned int *nextqrychar) {
  unsigned int numQueries = ctx->queries->count;
  int match_length = ctx->min_match_length;
...
void runPrintKernel(MatchContext *ctx, ReferencePage *page, MatchInfo *h_matches, unsigned int numMatches,
  ...
  fprintf(stderr, "Copying %d bytes to host memory for %d alignments\n", numAlignments * sizeof(Alignment),
          numAlignments);

  int DEBUG = 0;
  if (DEBUG) {
    for (int i = 0; i < numMatches; i++) {
      printf("m[%d]:\t%d\t%d\t%d\t%d\t%d\t%d\n", i, h_matches[i].resultsoffset, h_matches[i].queryid,
             h_matches[i].matchnode.data, h_matches[i].numLeaves, h_matches[i].edgematch, h_matches[i].qrystartpos);
    }
  ...
  printKernel<<<dimGrid, dimBlock, 0>>>(d_matches, numMatches, d_alignments,
...
#endif
                                        ctx->queries->d_addrs_tex_array, ctx->queries->d_lengths_array, page->begin,
                                        page->end, page->shadow_left, page->shadow_right, ctx->min_match_length
  ...
  CUDA_SAFE_CALL(cudaMemcpy((void *)alignments, (void *)d_alignments, alignmentSize, cudaMemcpyDeviceToHost));
  cudaDeviceSynchronize();
  stopTimer(atimer);
...
void runPrintOnCPU(MatchContext *ctx, ReferencePage *page, MatchInfo *h_matches, unsigned int numMatches,
                   Alignment *alignments, unsigned int numAlignments) {
  ...
  for (int i = 0; i < numMatches; ++i) {
    ...
    if (!(match.edgematch & FRMASK)) {
      printAlignments(page, alignments + match.resultsoffset,
                      ...
                      qrychars + addrs[qry],
#endif
                      qrylen, match.matchnode, match.qrystartpos, match.edgematch, min_match_length, 0,
...
void getExactAlignments(MatchContext *ctx, ReferencePage *page, bool on_cpu) {
  ...
  while (next_coord < last_coord) {
    ...
    startTimer(btimer);
    coordsToPrintBuffers(ctx, page, &h_matches, &h_alignments, boardFreeMemory, &next_coord, &numMatches,
    ...
    for (int m = 0; m < numMatches; m++) {
      ...
      for (int i = 0; i < h_matches[m].numLeaves; i++) {
        ...
        sprintf(buf, "%d\t%d\t%d\n", h_alignments[base + i].left_in_ref, h_matches[m].qrystartpos + 1,
...
int getQueryBlock(MatchContext *ctx, size_t device_mem_avail) {
  ...
  getQueriesTexture(queries->qfile, &queryTex, &queryLen, &queryAddrs, &names, &queryLengths, &numQueries,
                    &num_match_coords, device_mem_avail, ctx->min_match_length, ctx->reverse || ctx->forwardreverse);
...
void writeStatisticsFile(Statistics *stats, char *stats_filename, char *node_hist_filename = NULL,
...
void matchOnCPU(MatchContext *ctx, bool doRC) {
  ...
  if (doRC) {
    ...
    computeGold(&ctx->results, ctx->ref->str, ctx->queries->h_tex_array, ctx->queries->h_addrs_tex_array,
                ctx->queries->h_lengths_array, (PixelOfNode *)(ctx->ref->h_node_tex_array),
                (PixelOfChildren *)(ctx->ref->h_children_tex_array), ctx->queries->count, ctx->min_match_length,
                REVERSE);
  } else {
    computeGold(&ctx->results, ctx->ref->str, ctx->queries->h_tex_array, ctx->queries->h_addrs_tex_array,
                ctx->queries->h_lengths_array, (PixelOfNode *)(ctx->ref->h_node_tex_array),
...
void matchOnGPU(MatchContext *ctx, bool doRC) {
  ...
  if (doRC) {
    ...
    mummergpuRCKernel<<<dimGrid, dimBlock, 0>>>(ctx->results.d_match_coords, ctx->queries->d_tex_array,
...
void getMatchResults(MatchContext *ctx, unsigned int page_num) { transferResultsFromDevice(ctx); }

void matchQueryBlockToReferencePage(MatchContext *ctx, ReferencePage *page, bool reverse_complement) {
  char *ktimer = createTimer();

  fprintf(stderr, "Memory footprint is:\n\tqueries: %d\n\tref: %d\n\tresults: %d\n", ctx->queries->bytes_on_board,
          ctx->ref->bytes_on_board, ctx->results.bytes_on_board);
...
int matchSubset(MatchContext *ctx, ReferencePage *page) {
  ...
  fprintf(stderr, "Matching queries %s - %s against ref coords %d - %d\n", ctx->queries->h_names[0],
          ctx->queries->h_names[ctx->queries->count - 1], page->begin, page->end);
...
int streamReferenceAgainstQueries(MatchContext *ctx) {
  ...
  buildReferenceTexture(&(pages[0].ref), ctx->full_ref, pages[0].begin, pages[0].end, ctx->min_match_length,
                        ctx->dotfilename, ctx->texfilename, &(ctx->statistics));
  ...
  for (int i = 1; i < num_reference_pages - 1; ++i) {
    ...
    buildReferenceTexture(&(pages[i].ref), ctx->full_ref, pages[i].begin, pages[i].end, ctx->min_match_length, NULL,
                          NULL, &(ctx->statistics));
  ...
  if (num_reference_pages > 1) {
    ...
    buildReferenceTexture(&(pages[last_page].ref), ctx->full_ref, pages[last_page].begin, pages[last_page].end,
                          ctx->min_match_length, NULL, NULL, &(ctx->statistics));
