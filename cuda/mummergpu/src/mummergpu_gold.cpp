...
char getRefGold(int refpos, char *ref) { return ref[refpos]; }
...
SingleNode getMerged(PixelOfNode *nodes, PixelOfChildren *childrenarr, unsigned int cur, int getChildrenData
#if TREE_ACCESS_HISTOGRAM
                     ,
...
void set_result(unsigned int cur, MatchCoord *result, int edge_match_length, int qry_match_len, int min_match_len,
                int rc) {
...
int kernel_gold(int qryid, MatchResults *results, char *queries, PixelOfNode *nodes, PixelOfChildren *childrenarr,
                char *ref, const int *queryAddrs, const int *queryLengths, const int numQueries, const int min_match_len
#if TREE_ACCESS_HISTOGRAM
                ,
                int *node_hist, int *child_hist
#endif
...
void rc_kernel_gold(int qryid, MatchResults *results, char *refstr, char *queries, const int *queryAddrs,
                    const int *queryLengths, PixelOfNode *nodeTexture, PixelOfChildren *childrenTexture,
...
#pragma omp parallel for
    ...
    for (int i = 0; i < numQueries; ++i) {
      rc_kernel_gold(i, results, refstr, queries, queryAddrs, queryLengths, nodeTexture, childrenTexture, numQueries,
                     match_length);
    }
...
#pragma omp parallel for
    ...
    for (int i = 0; i < numQueries; ++i) {
      ...
      #endif
      */
      kernel_gold(i, results, queries, nodeTexture, childrenTexture, refstr, queryAddrs, queryLengths, numQueries,
                  match_length
#if TREE_ACCESS_HISTOGRAM
                  ,
                  int *node_hist, int *child_hist
