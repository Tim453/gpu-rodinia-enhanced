...
void printHelp() {
  fprintf(stderr, "Align a set of query strings to a reference sequence using the GPU.\n"
                  "\n"
...
int main(int argc, char *argv[]) {
  ...
  MatchContext ctx;
  if ((err = createMatchContext(&ref, &queries, 0, OPT_on_cpu, OPT_match_length, OPT_stats_file, OPT_reverse,
                                OPT_forwardreverse, OPT_forwardcoordinates, OPT_showQueryLength, OPT_dotfilename,
                                OPT_texfilename, &ctx))) {
    printStringForError(err);
    exit(err);
  }
