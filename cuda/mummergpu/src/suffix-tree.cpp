...
public:
...
#ifdef MPOOL
  void *operator new(size_t num_bytes, PoolMalloc_t *mem) { return mem->pmalloc(num_bytes); }
#endif

  SuffixNode(int s, int e, int leafid, SuffixNode *p, SuffixNode *x)
      : m_start(s), m_end(e), m_nodeid(++s_nodecount), m_leafid(leafid), m_numleaves(0), m_parent(p), m_suffix(x),
        m_printParent(NULL) {
  ...
  const char *str(const char *refstr) { return substr(refstr, m_start, m_end - m_start + 1); }
...
ostream &operator<<(ostream &os, SuffixNode *n) { return n->printNodeLabel(os); }
...
public:
  ...
  void printNodeDot(SuffixNode *node, ostream &dfile) {
    ...
    if (node->m_suffix) {
      dfile << " " << node << " -> " << node->m_suffix << " [style=dotted, constraint=false]" << endl;
  ...
  void printDot(const char *dotfilename) {
    ...
    dfile << " label=\"Suffix tree of \'" << m_string << "\' len:" << m_strlen - 1 << " nc:" << SuffixNode::s_nodecount
          << "\"" << endl;
  ...
  void buildUkkonen() {
    ...
    if (DEBUG) {
      cerr << "Building Ukkonen Tree for " << m_string << endl << "Len: " << len << endl;
    }
    ...
    for (int i = 2; i <= len; i++) {
      ...
      for (int j = startj; j <= i; j++) {
        ...
        if (DEBUG) {
          ...
          cerr << i << "." << j << " Phase " << i << " Extension " << j << ": \"" << beta << "\" bp:" << betapos
               << endl;
          ...
          if (!child) {
            ...
            if (DOJUMP) {
              ...
              if (mustmatch >= childlen) {
                ...
                if (DEBUG) {
                  cerr << i << "." << j << "   Edge Jump by: " << childlen << " new bp: " << betapos
                       << " np: " << nodepos << endl;
                }
                ...
                if (DEBUG) {
                  cerr << i << "." << j << "   Partial Jump by: " << mustmatch << " new bp: " << betapos
                       << " np: " << nodepos << endl;
                }
...
#if VERBOSE
              cerr << i << "." << j << "   child bp: " << betapos << "[" << m_string[betapos] << "] nb [" << nodebase
                   << "]" << endl;
#endif
                ...
                if (betapos == i + 1) {
                  ...
                  if ((nodewalk == child->len(i)) && (child->m_end == len)) {
                    ...
                    if (DOPHASETRICK) {
                      ...
                      if (DEBUG) {
                        cerr << i << "." << j << "    Implicit Extension... start next phase at " << startj
                             << ", saved " << skip << endl;
                      }
                    ...
                    if (DOINTERNALSKIP) {
                      ...
                      if (DEBUG) {
                        cerr << i << "." << j << "    Implicit Extension... skipping rest of phase, saved " << skip
                             << endl;
                      }
...
void buildSuffixTreeTexture(PixelOfNode **nodeTexture, PixelOfChildren **childrenTexture, unsigned int *width,
                            unsigned int *node_height, unsigned int *children_height, AuxiliaryNodeData **aux_data,
                            const char *refstr, int min_match_len) {
...
void printTreeTexture(const char *texfilename, PixelOfNode *nodeTexture, PixelOfChildren *childrenTexture,
                      int nodecount) {
  ...
  for (int i = 0; i < nodecount; i++) {
    ...
    texfile << i << "\t" << myaddress.x << "\t"
            ...
            << myaddress.y << "\t"
#endif
            << MK3(nodeTexture[i].start) << "\t" << MK3(nodeTexture[i].end) << "\t" << MK3(nodeTexture[i].depth)
            << "\t";
...
extern "C" void createTreeTexture(const char *refstr, PixelOfNode **nodeTexture, PixelOfChildren **childrenTexture,
                                  unsigned int *width, unsigned int *node_height, unsigned int *children_height,
                                  AuxiliaryNodeData **aux_data, int *num_nodes, int min_match_len,
...

{
  ...
  buildSuffixTreeTexture(nodeTexture, childrenTexture, width, node_height, children_height, aux_data, gtree->m_string,
                         min_match_len);
...
extern "C" void getReferenceString(const char *filename, char **refstr, size_t *reflen) {
  ...
  while (file >> buffer) {
    if (buffer[0] == '>') {
      cerr << endl << "ERROR: Only a single reference sequence is supported!" << endl;
...
extern "C" void getQueriesTexture(int qfile, char **queryTexture, size_t *queryTextureSize, int **queryAddrs,
                                  char ***queryNames, int **queryLengths, unsigned int *numQueries,
                                  unsigned int *num_match_coords, unsigned int memory_avail, int min_match_length,
                                  bool rc) {
  EventTime_t timer;
    ...
    for (; i < bytes_read; i++) {
      ...
      if (b == '>') {
        ...
        if (offsetspos == offsetssize) {
          ...
          if (!offsets || !lengths) {
            cerr << endl << "ERROR: Realloc failed: requested " << sizeof(int) * offsetssize << endl;
            exit(1);
          }
        }
        ...
        cerr << endl
             << "ERROR: Unexpected character: " << buf[i] << " in query file at byte: " << total_read + i << endl;
        exit(1);
      }
    }
  ...
  *num_match_coords = *queryTextureSize - *numQueries * (min_match_length + 1);
  ...
  cerr << offsetspos << " queries (" << qrylen << " bp), need " << curr_mem_usage << " bytes on the GPU ("
       << memory_avail << " avail) " << timer.str(true, 5) << endl;
}
...
#define GETNODE(node_addr)                                                                                             \
  (((PixelOfNode *)(page->ref.h_node_tex_array)) + (node_addr.x) + (node_addr.y * MAX_TEXTURE_DIMENSION))
#define GETCHILDREN(node_addr)                                                                                         \
  (((PixelOfChildren *)(page->ref.h_children_tex_array)) + (node_addr.x) + (node_addr.y * MAX_TEXTURE_DIMENSION))
...
void printNodeAlignments(const ReferencePage *page, const char queryflankingbase, const TextureAddress node,
                         const int qrypos, int qrylen, const pathblock path[], int path_idx, bool rc) {
  ...
  if (isLeaf) {
    ...
    if (child.data) {
      printNodeAlignments(page, queryflankingbase, child, qrypos, qrylen, path, path_idx, rc);
    }
...
void printAlignmentsOld(ReferencePage *page, char *query, int qrylen, int nodeid, int qrypos, int edge_match,
                        int min_match, bool rc, bool forwardcoordinates) {
  ...
  printNodeAlignments(page, flankingbase, prev, qrypos + 1, MK3(SDEPTH(prev)), path, path_idx - 1, rc);
}
...
// Stackless printing
void printAlignments(ReferencePage *page, Alignment *h_alignments, char *query, int qrylen,
                     TextureAddress matchNodeAddr, int qrystartpos, int edge_match, int min_match, bool rc,
  ...
  if (verbose) {
    ...
    cout << "starting node: " << PADDR(cur) << " depth: " << MK3(SDEPTH(cur)) << endl;
  }
  ...
  if (printParent.data != NODE_PRINTPARENT(matchNodeId).data) {
    ...
    cout << "printParent (" << PADDR(printParent) << "):" << MK3(SDEPTH(printParent)) << " != NODE_PRINTPARENT ("
