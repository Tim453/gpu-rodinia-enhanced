...
#if TWO_LEVEL_NODE_TREE
  int id = addr2id(cur);
  if (use_two_level && id < NODE_THRESH) {
    return node_tree_top[id].data;
  }
#endif
...
__device__ void printNode(int nodeid
  ...
  unsigned int a;
  arrayToAddress(cd.a, a);
  unsigned int c;
  arrayToAddress(cd.c, c);
  unsigned int g;
  arrayToAddress(cd.g, g);
  unsigned int t;
  arrayToAddress(cd.t, t);
  unsigned int d;
  arrayToAddress(cd.d, d);

  unsigned int p;
  arrayToAddress(nd.parent, p);
  unsigned int s;
  arrayToAddress(nd.suffix, s);
  ...
  XPRINTF("%d\t" fNID "\t%d\t%d\t%d\t%d\t" fNID "\t" fNID "\t" fNID "\t" fNID "\t" fNID "\t" fNID "\t" fNID "\n",
          nodeid, NID(addr), start, end, depth, leafchar, NID(a), NID(c), NID(g), NID(t), NID(d), NID(p), NID(s));
}
...
__device__ void set_result(unsigned int cur, _MatchCoord *result, int edge_match_length, int qry_match_len,
                           int min_match_len, int rc
#if VERBOSE
#if !NODETEX
                           ,
                           _PixelOfNode *nodes
...
#if VERBOSE
    _PixelOfNode nd;
    nd.data = GETNODE(cur, false);

    XPRINTF("  saving match cur=%d " fNID " len=%d edge_match=%d depth=%d\n", result->data.x, NID(cur), qry_match_len,
            edge_match_length, MKI(nd.depth));
...
__global__ void mummergpuKernel(void *match_coords,
#if COALESCED_QUERIES
                                int *coordAddrs,
#endif
                                ...
                                char *ref,
#endif
                                const int *queryAddrs, const int *queryLengths, const int numQueries,
  ...
  RECORD_RESULT: {
    ...
    SET_RESULT(cur, result, refpos - node_start, qry_match_len, min_match_len, FORWARD);
...
__global__ void mummergpuRCKernel(MatchCoord *match_coords, char *queries, const int *queryAddrs,
                                  const int *queryLengths, const int numQueries, const int min_match_len) {
  /*
     int qryid = __umul24(blockIdx.x,blockDim.x) + threadIdx.x;
...
__global__ void printKernel(MatchInfo *matches, int totalMatches, Alignment *alignments,
#if !QRYTEX
#if COALESCED_QUERIES
                            int *queries,
#else
                            ...
                            _PixelOfChildren *childrenarr,
#endif
                            const int *queryAddrs, const int *queryLengths, const int page_begin, const int page_end,
                            const int page_shadow_left, const int page_shadow_right, const int min_match_length
      ...
      if (isLeaf && isLeaf != queryflankingbase) {
        ...
        if ((left_in_ref != page_begin || page_shadow_left == -1) &&
          ...
          if (!(left_in_ref > page_begin && right_in_ref < page_shadow_left)) {
            ...
            XPRINTF("%8d%10d%10d\n", left_in_ref, matches[matchid].qrystartpos + 1, matchlen);
