...
// previousely allocated for results), every next step is incremented by 1 time unit (h_init). 	4) Function assumes that
// time interval starts at 0 (xmin) and ends at integer value (xmax) specified by the uses as a parameter on command
// line.
// 5) The appropriate amount of memory is previousely allocated for that range (y).

//	5) This setup in 3) - 5) allows solver to adjust the step ony from current time instance to current time
// instance + 0.9. The next time instance is current time instance + 1;

//	6) Solver also takes parameters (params) that it then passes to the equations.

//	7) The original solver cannot handle cases when equations return NAN and INF values due to discontinuities and
/// 0. That is why equations provided by user need to make sure that no NAN and INF are returned.
...
int solver(fp **y, fp *x, int xmax, fp *params, fp *com,
  ...
  // Verify that the step size is positive and that the upper endpoint of integration is greater than the initial
  // enpoint.               //
  if (xmax < xmin || h <= 0.0) {
    return -2;
  }

  // If the upper endpoint of the independent variable agrees with the initial value of the independent variable.  Set
  // the value of the dependent variable and return success. //
  if (xmax == xmin) {
    return 0;
  }
  ...
  for (k = 1; k <= xmax; k++) { // start after initial value
    ...
    for (j = 0; j < ATTEMPTS; j++) {
      ...
      // limit step to 0.9, because when it gets close to 1, it no longer makes sense, as 1 is already the next time
      // instance (added to original algorithm)
      if (h >= 0.9) {
        h = 0.9;
      }
