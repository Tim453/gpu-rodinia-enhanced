...
__global__ void solver_2(int workload, int xmax,
                         ...
                         fp *com, fp *err, fp *scale, fp *yy, fp *initvalu_temp, fp *finavalu_temp) {
  ...
  if (tid < workload) {
    ...
    for (k = 1; k < (xmax + 1); k++) { // start after initial value
      ...
      x[x_pointer_current] = (fp)k; // set this to k if you want time incremente with respect to previous to be k+h, set
                                    // this to k-1 if you want the increment to be h
      h = h_init;
