...
//	MAIN FUNCTION
//=====================================================================
__device__ void kernel_ecc_2(fp timeinst, fp *d_initvalu, fp *d_finavalu, int valu_offset, fp *d_params) {
  ...
  // fp junctionLength;
  // // junc length [um] fp junctionRadius;
  // // junc radius [um] fp distSLcyto;
  // // dist. SL to cytosol [um] fp distJuncSL;
  // // dist. junc to SL [um] fp DcaJuncSL;
  // // Dca junc to SL [cm^2/sec] fp DcaSLcyto;
  // // Dca SL to cyto [cm^2/sec] fp DnaJuncSL;
  // // Dna junc to SL [cm^2/sec] fp DnaSLcyto;
  // // Dna SL to cyto [cm^2/sec]
  fp Vcell; // [L]
  fp Vmyo;
  fp Vsr;
  fp Vsl;
  ...
  // // junc length [um] junctionRadius = 15e-3;
  // // junc radius [um] distSLcyto = 0.45;
  // // dist. SL to cytosol [um] distJuncSL = 0.5;
  // // dist. junc to SL [um] DcaJuncSL = 1.64e-6;
  // // Dca junc to SL [cm^2/sec] DcaSLcyto = 1.22e-6;
  // // Dca SL to cyto [cm^2/sec] DnaJuncSL = 1.09e-5;
  // // Dna junc to SL [cm^2/sec] DnaSLcyto = 1.79e-5;
  // // Dna SL to cyto [cm^2/sec]
  ...
  // SAjunc = 20150*pi*2*junctionLength*junctionRadius; // [um^2] SAsl = pi*2*cellRadius*cellLength;
  // // [um^2]
  ...
  // Bmax_CaM = 24e-3;
  // // [mM], CaM buffering koff_cam = 238e-3;
  // // [1/ms] kon_cam = 34;
  // // [1/mM/ms]
  ...
  if (d_initvalu_39 >= -40) {
    ...
    aj = (-127140 * exp(0.2444 * d_initvalu_39) - 3.474e-5 * exp(-0.04391 * d_initvalu_39)) * (d_initvalu_39 + 37.78) /
         (1 + exp(0.311 * (d_initvalu_39 + 79.23)));
  ...
  tauxr = 1 / (0.00138 * (d_initvalu_39 + 7) / (1 - exp(-0.123 * (d_initvalu_39 + 7))) +
  ...
  tauxs = 1 / (7.19e-5 * (d_initvalu_39 + 30) / (1 - exp(-0.148 * (d_initvalu_39 + 30))) +
  ...
  bki = (0.49124 * exp(0.08032 * (d_initvalu_39 + 5.476 - ek)) + exp(0.06175 * (d_initvalu_39 - ek - 594.31))) /
  ...
  ibarca_sl = pCa * 4 * (d_initvalu_39 * Frdy * FoRT) *
              ...
              (exp(2 * d_initvalu_39 * FoRT) - 1);
  ibark = pK * (d_initvalu_39 * Frdy * FoRT) * (0.75 * d_initvalu_35 * exp(d_initvalu_39 * FoRT) - 0.75 * Ko) /
          (exp(d_initvalu_39 * FoRT) - 1);
  ibarna_j = pNa * (d_initvalu_39 * Frdy * FoRT) * (0.75 * d_initvalu_32 * exp(d_initvalu_39 * FoRT) - 0.75 * Nao) /
             (exp(d_initvalu_39 * FoRT) - 1);
  ibarna_sl = pNa * (d_initvalu_39 * Frdy * FoRT) * (0.75 * d_initvalu_33 * exp(d_initvalu_39 * FoRT) - 0.75 * Nao) /
              (exp(d_initvalu_39 * FoRT) - 1);
  ...
  I_CaK = (ibark * d_initvalu_4 * d_initvalu_5 * (Fjunc_CaL * (1 - d_initvalu_6) + Fsl_CaL * (1 - d_initvalu_7)) *
           pow(Q10CaL, Qpow)) *
          0.45;
  ...
  s3_junc = (KmCai * pow(Nao, 3) * (1 + pow((d_initvalu_32 / KmNai), 3)) + pow(KmNao, 3) * d_initvalu_36 +
             pow(KmNai, 3) * Cao * (1 + d_initvalu_36 / KmCai) + KmCao * pow(d_initvalu_32, 3) +
             pow(d_initvalu_32, 3) * Cao + pow(Nao, 3) * d_initvalu_36) *
            (1 + ksat * exp((nu - 1) * d_initvalu_39 * FoRT));
  s2_sl = exp((nu - 1) * d_initvalu_39 * FoRT) * pow(Nao, 3) * d_initvalu_37;
  s3_sl = (KmCai * pow(Nao, 3) * (1 + pow((d_initvalu_33 / KmNai), 3)) + pow(KmNao, 3) * d_initvalu_37 +
           pow(KmNai, 3) * Cao * (1 + d_initvalu_37 / KmCai) + KmCao * pow(d_initvalu_33, 3) +
           pow(d_initvalu_33, 3) * Cao + pow(Nao, 3) * d_initvalu_37) *
          (1 + ksat * exp((nu - 1) * d_initvalu_39 * FoRT));
  ...
  // I_pca: Sarcolemmal Ca Pump Current
  I_pca_junc = Fjunc * pow(Q10SLCaP, Qpow) * IbarSLCaP * pow(d_initvalu_36, fp(1.6)) /
               (pow(KmPCa, fp(1.6)) + pow(d_initvalu_36, fp(1.6)));
  I_pca_sl = Fsl * pow(Q10SLCaP, Qpow) * IbarSLCaP * pow(d_initvalu_37, fp(1.6)) /
             (pow(KmPCa, fp(1.6)) + pow(d_initvalu_37, fp(1.6)));
  ...
  d_finavalu[offset_18] =
      kon_na * d_initvalu_33 * (Bmax_Nasl - d_initvalu_18) - koff_na * d_initvalu_18; // NaBsl     [mM/ms]

  // Cytosolic Ca Buffers
  d_finavalu[offset_19] =
      kon_tncl * d_initvalu_38 * (Bmax_TnClow - d_initvalu_19) - koff_tncl * d_initvalu_19; // TnCL      [mM/ms]
  d_finavalu[offset_20] = kon_tnchca * d_initvalu_38 * (Bmax_TnChigh - d_initvalu_20 - d_initvalu_21) -
                          koff_tnchca * d_initvalu_20; // TnCHc     [mM/ms]
  d_finavalu[offset_21] =
      kon_tnchmg * Mgi * (Bmax_TnChigh - d_initvalu_20 - d_initvalu_21) - koff_tnchmg * d_initvalu_21; // TnCHm [mM/ms]
  ...
  d_finavalu[offset_24] =
      kon_myomg * Mgi * (Bmax_myosin - d_initvalu_23 - d_initvalu_24) - koff_myomg * d_initvalu_24; // Myosin_mg [mM/ms]
  d_finavalu[offset_25] = kon_sr * d_initvalu_38 * (Bmax_SR - d_initvalu_25) - koff_sr * d_initvalu_25; // SRB [mM/ms]
  J_CaB_cytosol = d_finavalu[offset_19] + d_finavalu[offset_20] + d_finavalu[offset_21] + d_finavalu[offset_22] +
                  d_finavalu[offset_23] + d_finavalu[offset_24] + d_finavalu[offset_25];

  // Junctional and SL Ca Buffers
  d_finavalu[offset_26] =
      kon_sll * d_initvalu_36 * (Bmax_SLlowj - d_initvalu_26) - koff_sll * d_initvalu_26; // SLLj      [mM/ms]
  d_finavalu[offset_27] =
      kon_sll * d_initvalu_37 * (Bmax_SLlowsl - d_initvalu_27) - koff_sll * d_initvalu_27; // SLLsl     [mM/ms]
  d_finavalu[offset_28] =
      kon_slh * d_initvalu_36 * (Bmax_SLhighj - d_initvalu_28) - koff_slh * d_initvalu_28; // SLHj      [mM/ms]
  d_finavalu[offset_29] =
      kon_slh * d_initvalu_37 * (Bmax_SLhighsl - d_initvalu_29) - koff_slh * d_initvalu_29; // SLHsl     [mM/ms]
  ...
  d_finavalu[offset_30] =
      kon_csqn * d_initvalu_31 * (Bmax_Csqn - d_initvalu_30) - koff_csqn * d_initvalu_30; // Csqn      [mM/ms]
  oneovervsr = 1 / Vsr;
