...
float pFL(Points *points, int *feasible, int numfeasible, float z, long *k, int kmax, float cost, long iter, float e,
...
float pkmedian(Points *points, long kmin, long kmax, long *kfinal, int pid, pthread_barrier_t *barrier) {
  ...
  while (1) {
    ...
    //    pthread_barrier_wait(barrier);
    lastcost = cost;
    cost = pFL(points, feasible, numfeasible, z, &k, kmax, cost, (long)(ITER * kmax * log((float)kmax)), 0.1, pid,
               barrier);
    ...
    if (((k <= (1.1) * kmax) && (k >= (0.9) * kmin)) || ((k <= kmax + 2) && (k >= kmin - 2))) {
      ...
      cost = pFL(points, feasible, numfeasible, z, &k, kmax, cost, (long)(ITER * kmax * log((float)kmax)), 0.001, pid,
                 barrier);
    }
