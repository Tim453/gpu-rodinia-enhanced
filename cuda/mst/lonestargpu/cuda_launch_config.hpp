...
// RODINIA: the hand-rolled occupancy model above only knows architectures up to
// Kepler and falls back to guesses beyond that. mst sizes its GlobalBarrier grid
// from this number, and a barrier wider than the resident grid deadlocks, so ask
// the runtime instead -- it knows every architecture.
template<typename T>
inline __host__
std::size_t maximum_residency(T t, size_t CTA_SIZE, size_t dynamic_smem_bytes)
{
  int num_blocks = 0;
  cudaError_t err = cudaOccupancyMaxActiveBlocksPerMultiprocessor(&num_blocks, t, (int)CTA_SIZE,
                                                                  dynamic_smem_bytes);

  if (err != cudaSuccess)
    return 0;

  return (std::size_t)num_blocks;
}
