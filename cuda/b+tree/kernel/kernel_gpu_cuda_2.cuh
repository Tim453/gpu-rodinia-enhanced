...
__global__ void findRangeK(long height, knode *knodesD, long knodes_elem, long *currKnodeD, long *offsetD,
                           long *lastKnodeD, long *offset_2D, int *startD, int *endD, int *RecstartD, int *ReclenD) {
  ...
  for (i = 0; i < height; i++) {
    ...
    if ((knodesD[currKnodeD[bid]].keys[thid] <= startD[bid]) &&
      ...
      // "offset[bid]" calculated below that later addresses part of knodes goes outside of its bounds cause
      // segmentation fault more specifically, values saved into knodes->indices in the main function are out of bounds
      // of knodes that they address
    ...
    if ((knodesD[lastKnodeD[bid]].keys[thid] <= endD[bid]) && (knodesD[lastKnodeD[bid]].keys[thid + 1] > endD[bid])) {
      // this conditional statement is inserted to avoid crush due to but in original code
      // "offset_2[bid]" calculated below that later addresses part of knodes goes outside of its bounds cause
      // segmentation fault more specifically, values saved into knodes->indices in the main function are out of bounds
      // of knodes that they address
