...
// 2012.10; Ke Wang; Change it to non-interactive mode. Use command option read command from file. And also add output
// for easy verification among different platforms and devices.Merged into Rodinia main distribution 2.2.
//======================================================================================================================================================150
//	DESCRIPTION
//======================================================================================================================================================150
...
// j <x> <y> -- Run a range search of <x> bundled queries on the CPU and GPU (B+Tree) with the range of each search of
// size <y> x <z> -- Run a single search for value z on the GPU and CPU y <a> <b> -- Run a single range search for range
// a-b on the GPU and CPU q -- Quit. (Or use Ctl-D.)
...
// #include <sys/time.h>							// (in directory known to compiler)
// needed by ???
...
void list_iterator_init(list_t *l, list_iterator_t *li) { *li = l ? l->head : NULL; }
...
void *list_iterator_get_datum(list_iterator_t *li) { return *li ? (*li)->datum : NULL; }

bool list_iterator_is_valid(list_iterator_t *li) { return (*li != NULL); }

void list_reverse_iterator_init(list_t *l, list_reverse_iterator_t *li) { *li = l ? l->tail : NULL; }
...
void *list_reverse_iterator_get_datum(list_reverse_iterator_t *li) { return *li ? (*li)->datum : NULL; }

bool list_reverse_iterator_is_valid(list_reverse_iterator_t *li) { return (li != NULL); }
...
/* Utility function to give the height of the tree, which length in number of edges of the path from the root to any
 * leaf. */
 ...
 * separate nodes. With the verbose_output flag set. the values of the pointers corresponding to the keys also appear
...
/* Traces the path from the root to a leaf, searching by key.  Displays information about the path if the verbose flag
 * is set. Returns the leaf containing the given key. */
...
/* Helper function used in insert_into_parent to find the index of the parent's pointer to the node to the left of the
 * key to be inserted. */
...
/* Inserts a new key and pointer to a new record into a leaf so as to exceed the tree's order, causing the leaf to be
 * split in half. */
...
/* Inserts a new key and pointer to a node into a node into which these can fit without violating the B+ tree
 * properties. */
...
/* Inserts a new key and pointer to a node into a node, causing the node's size to exceed the order, and causing the
 * node to split into two. */
...
/* Master insertion function. Inserts a key and an associated value into the B+ tree, causing the tree to be adjusted
 * however necessary to maintain the B+ tree properties. */
...
/* Utility function for deletion. Retrieves the index of a node's nearest neighbor (sibling) to the left if one exists.
...
/* Coalesces a node that has become too small after deletion with a neighboring node that can accept the additional
...
/* Redistributes entries between two nodes when one has become too small after deletion but its neighbor is too big to
 * append the small node's entries without exceeding the maximum */
...
/* Deletes an entry from the B+ tree. Removes the record and its key and pointer from the leaf, and then makes all
...
int main(int argc, char **argv) {
  ...
  while (sscanf(commandPointer, "%c", &instruction) != EOF) {
    ...
    case 'k': {
      ...
      // CUDA kernel
      kernel_gpu_cuda_wrapper(records, records_mem, knodes, knodes_elem, knodes_mem,
    ...
    case 'j': {
      ...
      kernel_gpu_cuda_wrapper_2(knodes, knodes_elem, knodes_mem,
                                ...
                                currKnode, offset, lastKnode, offset_2, start, end, recstart, reclength);
