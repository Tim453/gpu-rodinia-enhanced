...
extern "C" void cpu_vlc_encode(unsigned int *indata, unsigned int num_elements, unsigned int *outdata,
                               unsigned int *outsize, unsigned int *codewords, unsigned int *codewordlens) {
      ...
      while (numbits > 0) {
        ...
        if (numbits == writebits)
          mask32 = (cw32 & ((1 << numbits) - 1))
                   << (32 - startbit - numbits); // first make sure that the start of the word is clean, then shift to
                                                 // the left as many places as you need
        else
          mask32 = cw32 >> (numbits - writebits); // shift out the bits that can not fit
