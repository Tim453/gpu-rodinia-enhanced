 ...
 * Huffman encoding when using tree generation with restriction on the size (max. Huffman codeword dize = 2x original
 * symbol size).
 *
...
/* HUFFMAN-FRIENDLY PAVLE
   ...
   -	NUMBER OF THREADS PER BLOCK IS 256; IF YOU WANT TO PLAY WITH DIFFERENT NUMBERS, THE CW CACHING SHOULD BE
   MODIFIED (SEE DPT* KERNELS) -	SM usage: 1x size of the input data (REUSE) + size of CWLUT TURN ON CACHING FOR
   HIGH ENTROPY DATA!
*/

__global__ static void vlc_encode_kernel_sm64huff(unsigned int *data, const unsigned int *gm_codewords,
  ...
  // if (wrbits == 32) as[kc] = tmpcw32;				//unnecessary overhead; increases number of
  // branches else
