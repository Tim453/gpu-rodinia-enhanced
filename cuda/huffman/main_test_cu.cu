...
void runVLCTest(char *file_name, uint num_block_threads, uint num_blocks) {
  ...
  cpu_vlc_encode((unsigned int *)sourceData, num_elements, (unsigned int *)crefData, &refbytesize, codewords,
                 codewordlens);
  ...
  pack2<<<num_scan_elements / 16, 16>>>((unsigned int *)d_destData, d_cindex, d_cindex2,
