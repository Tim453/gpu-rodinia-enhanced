 ...
 *  Second atomic operation actually sets these bits to the value stored in the codeword; the other bits are left
 * unotuched First atomic operation is a necessary prepration - we change only the bits that will be affected by the
 * codeword to be written to 1s in order for set bits to work with using atomicand.
 *  TODOs: benchmark performance 1) gm atomics vs sm atomics; 2) memset at init time vs. atomicOr
 */
__device__ void static put_bits_atomic2(unsigned int *out, unsigned int kc, unsigned int startbit, unsigned int numbits,
  ...
  mask <<= restbits; // fill in zeros at the back positions -> 0000...001111000 -> 11111110000111111111111 (in order not
                     // to and other positions)
 ...
 */
__device__ void static put_bits_atomic2a(unsigned int *out, unsigned int kc, unsigned int startbit,
                                         unsigned int numbits, unsigned int codeword) {
  unsigned int cw32 = codeword;
  unsigned int restbits = 32 - startbit - numbits;
