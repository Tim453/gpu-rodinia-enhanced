...
static void prescanArrayRecursive(unsigned int *outArray, const unsigned int *inArray, int numElements, int level) {
  unsigned int blockSize = BLOCK_SIZE; // max size of the thread blocks
  ...
  unsigned int sharedMemSize = sizeof(unsigned int) * (numEltsPerBlock + extraSpace);
