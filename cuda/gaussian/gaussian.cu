...
void PrintDeviceProperties() {
  ...
  for (int nDeviceIdx = 0; nDeviceIdx < nDevCount; ++nDeviceIdx) {
    ...
    if (cudaSuccess == cudaGetDeviceProperties(&deviceProp, nDeviceIdx)) {
      ...
      printf("\nMaximum Thread Dimension (block) \t - %d %d %d", deviceProp.maxThreadsDim[0],
             deviceProp.maxThreadsDim[1], deviceProp.maxThreadsDim[2]);
      printf("\nMaximum Thread Dimension (grid) \t - %d %d %d", deviceProp.maxGridSize[0], deviceProp.maxGridSize[1],
...
__global__ void Fan1(float *m_cuda, float *a_cuda, int Size, int t) {
  ...
  *(m_cuda + Size * (blockDim.x * blockIdx.x + threadIdx.x + t + 1) + t) =
      *(a_cuda + Size * (blockDim.x * blockIdx.x + threadIdx.x + t + 1) + t) / *(a_cuda + Size * t + t);
}
